...
| `cut(from, to)` | Restrict content to the half-open range `[from, to)`. Positions are local to this node (its token at 0, children from 1), the same space `replace` uses, so `cut` keeps exactly what `replace` removes. `cut(0, node_size)` selects the whole node and returns `self`; any other range returns a new node.
