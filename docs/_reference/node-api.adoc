...
| `cut(from, to)` | Return a copy restricted to `[from, to)`. Positions are local to this node (its token at 0, children from 1), the same space `replace` uses. `cut` keeps exactly what `replace` removes.
